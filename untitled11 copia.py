...
Original file is located at
    https://colab.research.google.com/drive/18mx1VqeujabETKFMsGxXZjhC4jr39rDb
"""
...
def eliminar_puntuacion(texto):
    # 1) Cada signo de puntuación (todo lo que no sea letra/dígito/espacio) → espacio
...
def eliminar_acentos(texto):
    return unidecode(texto)  # Elimina los acentos
...
# 3) Generar la WordCloud pasando mis_stopwords, no STOPWORDS
wordcloud = WordCloud(
    width=3000,
    height=2000,
    random_state=1,
    ...
    colormap='Set2',
    collocations=False,
    stopwords=mis_stopwords        # ← aquí
).generate(texto_lemas)
...
# 1) Prepara tu lista de documentos
...
# 1) Asegúrate de tener df_agrupado cargado en memoria y que
#    su columna 'Texto lematizado' exista.

# 2) Construye la lista de documentos (texto lematizado) sin NaN
docs = df_agrupado['Texto lematizado'].dropna().tolist()

# 3) Convierte tu set de stopwords a lista
mis_stopwords_list = list(mis_stopwords)

# 4) Configura CountVectorizer para **solo bigramas**, eliminando acentos justo antes:
...
# 5) Ajusta y transforma el corpus
X_bi = cv_bi.fit_transform(docs)

# 6) Extrae bigramas y recuentos totales
bigrams = cv_bi.get_feature_names_out()
counts_bi = X_bi.sum(axis=0).A1

# 7) Crea DataFrame ordenado por recuento
df_bigrams = pd.DataFrame({
    'Bigrama': bigrams,
    'Recuento': counts_bi
}).sort_values('Recuento', ascending=False)

# 8) Selecciona los 30 más frecuentes
...
# 1) Asegúrate de que df_agrupado y mis_stopwords_list estén definidos.
docs = df_agrupado['Texto lematizado'].dropna().tolist()
mis_stopwords_list = list(mis_stopwords)

# 2) Configura CountVectorizer para **solo trigramas**, eliminando acentos antes:
...
# 3) Ajusta y transforma el corpus
X_tri = cv_tri.fit_transform(docs)

# 4) Extrae trigramas y sus recuentos totales
trigrams = cv_tri.get_feature_names_out()
counts_tri = X_tri.sum(axis=0).A1

# 5) Crea DataFrame ordenado por recuento
df_trigrams = pd.DataFrame({
    'Trigrama': trigrams,
    'Recuento': counts_tri
}).sort_values('Recuento', ascending=False)

# 6) Selecciona los 30 más frecuentes
...
# 5) Asignar ticks equidistantes con tus labels
...
# 0) Antes de todo, asegúrate de que 'Año' es numérico
...
print(f"Coeficiente de determinación (r²): {r2:.3f} ({r2*100:.1f}% de varianza explicada)")

# 3) Cálculo rápido con pandas (solo r)
r_pandas = ideol.corr(long)
print(f"\nCorrelación con pandas: {r_pandas:.3f}")
...
#aqui lo que hacemos es eliminar outliers#
import seaborn as sns
import matplotlib.pyplot as plt
...
# 0) Partimos de df_agrupado con la columna 'Nº caracteres/página' ya limpia
...
# 1) Media de ideología por partido (para el orden)
ideol = df_agrupado.groupby('Partido')['Ideología'].mean().sort_values()
